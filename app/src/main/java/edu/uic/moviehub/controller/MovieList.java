...
import edu.uic.moviehub.adapter.MovieAdapter;
            ...
            if (loading) {
                if (currentPage != totalPages) {
                    ...
                    new Thread(new Runnable() {
                        public void run() {
                            try {
                                request.execute(MovieHub.url + getCurrentList() + "?&api_key=" + MovieHub.key + "&page=" + currentPage).get(10000, TimeUnit.MILLISECONDS);
        ...
        if (listView != null) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        if (!isLoading) {
                            request.execute(MovieHub.url + getCurrentList() + "?&api_key=" + MovieHub.key).get(10000, TimeUnit.MILLISECONDS);
                        }
                    } catch (TimeoutException | ExecutionException | InterruptedException e) {
                        request.cancel(true);
                        toastLoadingMore.cancel();
